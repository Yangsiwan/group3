package Java;
	...
	public	int main() {
		...
		char chara=(char) (rand.nextInt(16)+65);//랜덤으로 처음 알파벳 16개중 하나를 금지 알파벳으로 정한다.
		String ch=Character.toString(chara);//해당 알파벳을 밑의 함수들에서 사용하기 위해 String으로 바꿔준다.
		char Chara=(char)((int)chara+32); //대문자도 동일하게 만들어준다/
		...
		try (Scanner src = new Scanner(System.in)) {
			...
			long end=System.currentTimeMillis();//시간 측정을 종료함
		...
		else if(damage<10)damage+=Error-5;//10회 미만이면 5회만큼 제하고 데미지를 준다
		else damage+=4;//그보다 클시 앞선 것의 최대 대미지인 4만큼만 데미지를 준다.
		if(damage>4)damage=4;//4보다 데미지가 더 들어가게 되면 이를 수정한다.
		
		return damage;//데미지를 반환한다.
	}
}
